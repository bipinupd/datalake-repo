...
class DAGDependencyForGCSToBQTest(unittest.TestCase):
    @classmethod
    def setUp(self):
        os.environ['AIRFLOW_VAR_ENV'] = 'dev'
        os.environ['AIRFLOW_VAR_WORK_PROJECT'] = 'test'
        os.environ['COMPOSER_BUCKET'] = 'test'
        self.dagbag = DagBag()
        self.dag_id = 'DAG_WordCount'
    ...
    def test_contain_tasks(self):
        ...
        assert sorted(task_ids) == sorted([
            'start', 'move_file_to_processing', 'dataflow_java_app',
    ...
    def test_dependencies_of_print_date(self):
        ...
        dag = self.dagbag.get_dag(self.dag_id)
        dataflow_task = dag.get_task('dataflow_java_app')
